...
st.header('Sector Price per sqft GeoMap of GurGaon')
...
Sector_Word_df=pd.read_csv('Sector_Word_df')
st.divider()
st.header('Amenities of Gurgaon')
# showing the wordcloud here
...
st.header('Amenities Sector-wise')
word=st.selectbox('Sector',Sector_Word_df['sector'])
if(st.button('Amenities offered')):
    plt.rcParams["font.family"] = "Arial"
    wordcloud = WordCloud(width = 800, height = 800, 
                      background_color ='white', 
                      stopwords = set(['s']),  # Any stopwords you'd like to exclude
                      min_font_size = 10).generate(Sector_Word_df[Sector_Word_df['sector']==word]['features'].values[0])
